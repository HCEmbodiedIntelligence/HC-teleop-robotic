...
SIM_ARGS=(--robot_name hc_tj_description)
...
/usr/bin/python3 "${HC_IO_ROOT}/src/scripts/general_sim_robot_control_node_ros2.py" "${SIM_ARGS[@]}" &
SIM_PID=$!
/usr/bin/python3 "${SCRIPT_DIR}/teleop_arm_controller.py" \
  --config "${SCRIPT_DIR}/arm_teleop.yaml" --backend "${BACKEND}" &
TELEOP_PID=$!
if [[ "${BACKEND}" == "generic" || "${BACKEND}" == "v23" ]]; then
...
echo "HC-TJ simulation PID=${SIM_PID}; VR adapter PID=${TELEOP_PID}; backend=${BACKEND}; ROS_DOMAIN_ID=${ROS_DOMAIN_ID:-0}"
