...
class X1SimNode final : public rclcpp::Node
{
public:
  explicit X1SimNode(const rclcpp::NodeOptions & options)
  : Node("x1_sim_adapter", options)
  {
  ...
  void onCommand(const hc_teleop_interfaces::msg::JointCommand & message)
  {
    if (message.control_mode != hc_teleop_interfaces::msg::JointCommand::CONTROL_MODE_POSITION) {
      RCLCPP_WARN_THROTTLE(
        get_logger(), *get_clock(), 2000, "X1 sim only accepts position commands");
      return;
    }
...
RCLCPP_COMPONENTS_REGISTER_NODE(hc_adapter_x1::X1SimNode)
